...
let
  ...
  # Only used by the restoreTheme activation hook below and by the
  # colocated theme-switch package — both need the same state path.
  stateDir = "\${XDG_STATE_HOME:-$HOME/.local/state}/theming";
  stateFile = "${stateDir}/active-theme";

  # Preset color themes with the full 19-key color set matugen also
  # produces, so theme-switch can apply either a wallpaper-derived or a
  # preset scheme through the same colors.json shape.
  presets = {
    catppuccin-mocha = {
      background = "#1e1e2e";
      foreground = "#cdd6f4";
  ...
  moduleBody = { pkgs, lib, inputs, self, ... }: {
    config = {
      # No nixpkgs package for matugen — comes from the pinned flake input.
      home.packages = [
        inputs.matugen.packages.${pkgs.stdenv.hostPlatform.system}.default
        self.packages.${pkgs.stdenv.hostPlatform.system}.theme-switch
...
in
{
  # theme-switch: applies a color scheme either from a wallpaper (via
  # matugen) or a built-in preset, writes ~/.cache/theme/colors.json
  # atomically, and regenerates Alacritty's colors.
  perSystem = { pkgs, ... }: {
    packages.theme-switch = pkgs.writeShellScriptBin "theme-switch" ''
      set -uo pipefail
  ...
  flake.homeModules.theming = moduleBody;
}
